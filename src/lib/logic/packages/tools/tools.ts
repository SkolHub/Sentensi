...
import { HandleGroupRotateModel, RotateTool } from '@/lib/logic/packages/tools/rotate-tool';
import { CreateGeneral } from '@/lib/logic/packages/generals/create-general';
import { HandleGroupMoveModel, HandleSelectModel, SelectTool } from '@/lib/logic/packages/tools/select-tool';
import { HandleGroupScaleModel, ScaleTool } from '@/lib/logic/packages/tools/scale-tool';
import { Point, Word } from '@/lib/logic/models';
import { getMedium, getSlope, groupRotateAngles, groupRotateLengths, pyth } from '@/lib/logic/math';
...
class Tools extends SentensiPackage<CreateGeneral> {
	...
	checkTools(point: Point): boolean {
		...
		if (this.rotate.activate(point)) {
			...
			this.general.details = {
				...
				angles: groupRotateAngles(this.general.details.target as Word[], this.rotate.state.track),
				lengths: groupRotateLengths(this.general.details.target as Word[], this.rotate.state.track),
				target: this.general.details.target
			};
	...
	handleTools(point: Point): boolean {
		switch (this.general.action) {
			case 'select':
				this.select.handleSelect(this.general.details as HandleSelectModel, point);

				return true;

			case 'groupMove':
				this.select.handleGroupMove(this.general.details as HandleGroupMoveModel, point);

				return true;

			case 'groupRotate':
				this.rotate.handleGroupRotate(this.general.details as HandleGroupRotateModel, point);

				return true;

			case 'groupScale':
				this.scale.handleGroupScale(this.general.details as HandleGroupScaleModel, point.x);
	...
	finishTools(): boolean {
		switch (this.general.action) {
			case 'select':
				const selectedWords = handleGroupSelect(this.general.words, this.select.state);
