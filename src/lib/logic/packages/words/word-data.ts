...
import { splitBezierIntoLengths, getPosAndAngle } from '@/lib/logic/packages/words/word-math';
...
export class WordData {
	...
	nextLetter() {
		const letterMetrics = this.ctx.measureText(this.word.content[++this.letterIndex]);

		for (
			let x1 = (this.prevLetterMetrics.width + letterMetrics.width) / 2 + this.gap, x2 = 0;
			x2 < x1;
