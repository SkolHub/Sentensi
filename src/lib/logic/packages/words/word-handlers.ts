...
import { contained, getPointOnCircle, getSlope, getXOnCircle, getYOnCircle, pyth } from '@/lib/logic/math';
...
export const handleGroupSelect = (words: Word[], selectBox: SelectBox): Word[] => {
	return words.filter(
		(word) =>
			contained(word.start, selectBox.x, selectBox.y, selectBox.width, selectBox.height) ||
			contained(
				{
